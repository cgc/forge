...
/**
 * Web-target stub for {@code forge.localinstance.properties.PreferencesStore}.
 *
 * <p>Identical to the original except that {@link #save()} is a no-op.  The
 * real implementation writes the preferences file via {@code FileWriter} →
 * {@code FileOutputStream}, which throws {@code FileNotFoundException} in the
 * browser because there is no writable local filesystem.
 *
 * <p>All preference <em>reads</em> still work: the constructor reads lines
 * from the named file via {@link FileUtil#readFile(String)} which is already
 * web-safe (returns an empty list when the file doesn't exist, causing all
 * prefs to fall back to their defaults).
 *
 * <p>Browser-local-storage persistence can be added in a future iteration.
 */
public abstract class PreferencesStore<T extends Enum<T> & PreferencesStore.IPref> {
    ...
    /**
     * No-op on the web target: there is no writable local filesystem in a
     * browser, so attempting to open a {@code FileWriter} throws
     * {@code FileNotFoundException}.  Preferences are held in memory for the
     * duration of the session only; browser-local-storage persistence can be
     * added later.
     */
    public void save() {
        // no-op on web target — no writable filesystem
    }
