...
/**
 * Web-target replacement for {@code forge.assets.TextRenderer}.
 *
 ...
 * class library.  The previous stub replaced the whole class with no-ops,
 * meaning <em>nothing in the game UI rendered text</em>.
 *
 * <p>This version is a faithful copy of the real implementation with a
 * single targeted change: the {@code java.text.BreakIterator} field is
 * replaced by the private inner class {@link SimpleLineBreaker}, which
 * detects line-break opportunities at ASCII whitespace and hyphen characters.
 * This is sufficient for English Magic card text; a full Unicode line-break
 * algorithm can be substituted later if needed.
 */
...
public class TextRenderer {
    ...
    /**
     * Replaces {@code BreakIterator.getLineInstance(Locale)} for the web target.
     * Finds break opportunities after ASCII whitespace and hyphens.
     */
    private final SimpleLineBreaker boundary = new SimpleLineBreaker();
                            ...
                            if (lastPiece instanceof TextPiece textPiece) {
                                ...
                                if (index != -1) {
                                    if (index == 0) {
                                        ...
                                        splitPiece.lineNum = textPiece.lineNum;
                                        pieces.add(lastPieceIdx + 1, splitPiece);
                                    }
                                    break;
                                }
                            }
    ...
    /**
     * Simple line-break-opportunity finder for the web target.
     *
     * <p>Replaces {@code java.text.BreakIterator.getLineInstance()} which is
     * absent from TeaVM's JavaScript class library.  Returns the index
     * <em>after</em> each whitespace or hyphen character in the text,
     * matching the positions that a standard line-break iterator would
     * return for typical ASCII / Latin card text.
     *
     * <p>The sentinel value {@code DONE} ({@code -1}) is returned after all
     * break positions have been consumed, matching
     * {@code BreakIterator.DONE}.
     */
    private static class SimpleLineBreaker {
        /** Equivalent to {@code BreakIterator.DONE}. */
        ...
        /** Returns the first break position (after the first whitespace/hyphen), or DONE. */
        int first() {
            pos = 0;
            return nextBreak();
        }

        /** Returns the next break position, or DONE when exhausted. */
        int next() {
            return nextBreak();
        }
        ...
        private int nextBreak() {
            while (pos < text.length()) {
                ...
                // Break opportunity: after a space, tab, or hyphen
                if (c == ' ' || c == '\t' || c == '-') {
                    return pos;
                }
            }
